/**
 * Progressive Blur Style - Advanced layered blur effects
 * Creates iOS/Windows-style blur glass effects with customizable layers
 *
...
function progressiveBlurStyle(target, options = {}) {
  const defaults = {
    // Blur Layer Settings
    blur: {
      enabled: true,
  ...
  clearBlurLayers(target);
  
  // Create layer container
  const layerContainer = document.createElement('div');
  layerContainer.className = 'blur-style-container';
  ...
  return {
    update: (newOptions) => progressiveBlurStyle(target, deepMerge(config, newOptions)),
    destroy: () => clearBlurLayers(target)
  };
}
...
function createBlurLayer(blurConfig, maskSettings) {
  ...
  blurContainer.className = 'blur-style-blur-container';
  ...
  blurValues.forEach((blur, i) => {
    ...
    blurDiv.className = `blur-style-layer-${i}`;
...
function createLuminosityLayer(lumConfig, maskSettings) {
  ...
  lumLayer.className = 'blur-style-luminosity-layer';
...
function createTintLayer(tintConfig, maskSettings) {
  ...
  tintLayer.className = 'blur-style-tint-layer';
...
function createNoiseLayer(noiseConfig, maskSettings) {
  ...
  noiseLayer.className = 'blur-style-noise-layer';
...
function clearBlurLayers(target) {
  const existing = target.querySelector('.blur-style-container');
  if (existing) {
    existing.remove();
  }
}
